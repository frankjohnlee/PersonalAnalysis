...
from qtpy.QtWidgets import QAbstractItemView, QItemDelegate, QMenu, QTableView
    ...
    def mousePressEvent(self, event):
        ...
        if event.button() == Qt.LeftButton and column == C.COL_ACTION:
            if model.is_installable(model_index):
                ...
                self.action_pressed(index)
                self.context_menu_requested(event)
        elif event.button() == Qt.RightButton and column == C.COL_ACTION:
            self.action_pressed(index)
            self.context_menu_requested(event)
        elif event.button() == Qt.LeftButton and column == C.COL_VERSION:
            if self.proxy_model is not None:
                if model.is_upgradable(model_index):
                    row_data = self.source_model.row(model_index.row())
                    name = row_data[C.COL_NAME]
                    versions = self.source_model.get_package_versions(name)
                    self.set_action_status(model_index, C.ACTION_UPGRADE,
                                           versions[-1])
